...
If the game version or language is not correctly set upon loading a save, and it has an active LocEnemy target, the target may get reset to a safe value to prevent a possible game crash. The game stores the current target encounter as an address value pointing directly to an encounter in the game data, and the location of that data shifts between each game version. Thus, the editor converts the encounter address to a standardized "encounter ID" (and writes the proper address for the target version when saving), but this can fail if the version or language was not correctly recognized at first. Also, not all GBA version revisions may be accounted for, so if those revisions have different addresses than expected, that could also cause issues.
